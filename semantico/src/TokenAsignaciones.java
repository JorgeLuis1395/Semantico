...
class TokenAsignaciones {
			...
			if (parametros.isEmpty()) {
				...
				for (int i = 0; i < parametros.size(); i++) {
					...
					if (parametros.get(i).image.equals(variable.image)) {
						...
						{
						System.out.println("Error Semantico: Linea: " + funcion.beginLine + " El parametro \" "
	...
	public static void verificarFuncion1(Token identificador) {
		...
		if (aux == false) {
			System.out.println("Error Semantico: Linea: " + identificador.beginLine + " La funcion \" "
		...
		if (j >= 0) {
			...
			for (Token y : parametros) {
				...
				if (y.image.equals(variable.image) && (y.kind == variable.kind)) {
					...
					aux = false;
					System.out.println("Error Semantico: Linea: " + funcion.beginLine + " La funcion \" "
			...
			System.out.println("Error Semantico: Linea: " + funcion.beginLine + " La funcion \" " + funcion.image
	...
	public static void insertarSimbolo(Token identificador) {
		...
		if (aux) { // verifica ya ya existe o no el simbolo a ingresar
			...
			System.out.println("Error Semantico: Linea: " + identificador.beginLine + " El identificador \" "
	...
	public static String checkAsing(Token TokenIzq, Token TokenAsig) {
		...
		if (TokenIzq.kind != 45 && TokenIzq.kind != 48) {
			...
			boolean sw=false;
				...
				{
				return "Error Semantico: Linea: " + TokenIzq.beginLine + " El identificador \"" + TokenIzq.image
		...
		if (TokenAsig.kind == 47) {
			...
			{
				return "Error Semantico: Linea: " + TokenAsig.beginLine + " El identificador " + TokenAsig.image
						+ " no ha sido declarado ";
			}
		...
		} else if (tipoIdent1 == 43) // string
		{
			if (strComp.contains(tipoIdent2))
				...
				return "Error Semantico: Linea: " + TokenIzq.beginLine + "No se puede convertir " + TokenAsig.image
						+ " a Cadena";
	...
	public static void validarRetornos(Token funcion, Token retorno) {
		...
		if (aux) {
			for (int i = 0; i < parametros.size(); i++) {
				if (parametros.get(i).image.equals(retorno.image)) {
					...
					if (parametros.get(i).kind != funcion.kind) {
						System.out.println("Error semantico: Linea: " + retorno.beginLine + " La funcion \""
				...
				for (int i = 0; i < tablaTokens.size(); i++) {
					if (tablaTokens.get(i).image.equals(retorno.image)) {
						...
						if (tablaTokens.get(i).kind != funcion.kind) {
							System.out.println("Error sem�ntico: Linea: " + retorno.beginLine + " La funcion \""
									+ funcion.image + "\" retorna tipo " + tipos(funcion.kind));
						}
						i = tablaTokens.size();
					}
			...
			System.out.println("Error semantico: Linea: " + retorno.beginLine + " La funcion \"" + funcion.image
					+ "\" no ha sido declarada");
		...
		if (variables.isEmpty()) {
			...
			for (int i = 0; i < variables.size(); i++) 
			{
					if (variables.get(i).image.equals(var.image) ) {
						...
						{
						System.out.println("Error Semantico: Linea: " + var.beginLine + " El identificador \" "
